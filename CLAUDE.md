...
clinvar-proto is the **generator** for the `clinvar_proto` package. See
...
- **The consumable artifact is generated, not committed.** `generated/` and
  ...
  `clinvar_transforms.yaml` (or re-vendor the XSD) and regenerate. To fix how
  the schema is shaped, the engine is `xsd-former`, not this repo.
- **Never hand-patch the XSD.** It is byte-identical to NCBI's published file
  and its MD5 is documented in the README; a local edit breaks that check.
  Schema shaping belongs in the transforms.
- **Version lives in `clinvar_transforms.yaml` (`build.version`)**, not in this
...
- **A new upstream XSD version means a new filename.** `ClinVar_VCV_<n>.xsd` is
  referenced from the `Makefile`, the release workflow, and the round-trip test;
  update all three when re-vendoring.
